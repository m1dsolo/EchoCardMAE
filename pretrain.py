...
from model.echocardmae_vit import EchoCardMAEViT
...
# get fg_mask for temporal-invariant alignment loss
roi = get_roi()
fg_mask = torch.tensor(roi.reshape(-1).copy(), device='cuda:0')
fg_mask = fg_mask.tile(8)
fg_mask = fg_mask.expand(BATCH_SIZE, -1)  # (B, 8*14*14)
...
# reconstruction denoising strategy
...
df = df[df['Split'] == 'TRAIN']  # only use training set
...
class MyModelModule(yd.ModelModule):
    ...
    def _step(self, x1, x2, y1, y2, mask):
        ...
        with autocast('cuda'):
            ...
            # temporal-invariant alignment loss
