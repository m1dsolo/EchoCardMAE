# --------------------------------------------------------
# Based on VideoMAE, BEiT, timm, DINO and DeiT code bases
...
class EchoCardMAEEncoder(nn.Module):
...
class EchoCardMAEDecoder(nn.Module):
    def __init__(
        self,
        patch_size: int = 16,
        embed_dim: int = 768,
...
class EchoCardMAEViT(nn.Module):
    def __init__(
        self,
        num_frames=16,
        img_size=224,
        ...
        self.encoder = EchoCardMAEEncoder(
            num_frames=num_frames,
            img_size=img_size, 
            patch_size=patch_size, 
        ...
        self.decoder = EchoCardMAEDecoder(
            patch_size=patch_size, 
        ...
        self.background_token = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))
    ...
    def forward(self, x, mask, fg_mask):
        ...
        pos_emd_bg = expand_pos_embed[~mask & ~fg_mask].reshape(B, -1, C)
        pos_emd_mask = expand_pos_embed[mask].reshape(B, -1, C)
        x_full = torch.cat([x_vis_decoder + pos_emd_vis, self.background_token.expand(B, -1, -1) + pos_emd_bg, self.mask_token + pos_emd_mask], dim=1) # [B, N, C_d]
